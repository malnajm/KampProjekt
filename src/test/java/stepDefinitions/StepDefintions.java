package stepDefinitions;

import common.Competitor;

import io.cucumber.java.en.Given;
import io.cucumber.java.en.Then;
import io.cucumber.java.en.When;
import org.junit.Assert;

public class StepDefintions {
    private Competitor compName = new Competitor();


    @Given("I have entered {string}")
    public void i_have_entered(String name) {

    }

    @Given("I have also entered {string}")
    public void i_have_also_entered(String city) {

    }

    @Given("I have typed {string}")
    public void i_have_typed(String country) {

    }

    @Given("I have written {string}")
    public void i_have_written(String gender) {

    }

    @Given("I have added {string}")
    public void i_have_added(String age) {

    }

    @When("I press enter")
    public void i_press_enter() {

    }

    @Then("they should be displayed on the screen")
    public void they_should_be_displayed_on_the_screen() {

    }
}
